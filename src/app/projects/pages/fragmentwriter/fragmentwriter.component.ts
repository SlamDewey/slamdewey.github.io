...
import { DEFAULT_SHADER_PROGRAMS, UNIFORM_NAMES } from "./shader-programs";
import parsedOpenGlDocs from "./parsedOpenGlDocs";
import { ActivatedRoute } from "@angular/router";
import { DropdownLinkData } from "src/app/components/dropdown-link-selector/dropdown-link-selector.component";
...
export class FragmentwriterComponent
  implements OnInit, AfterViewInit, ControlValueAccessor
{
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params) => {
      const shaderProgram = DEFAULT_SHADER_PROGRAMS.find(
        (program) => program.url === params["shader"],
      );
  ...
  private initMonaco(options: any, insideNg: boolean): void {
    ...
    monaco.languages.setMonarchTokensProvider("glsl", {
      ...
      builtinFunctions: Object.keys(parsedOpenGlDocs).filter(
        (k) => !k.startsWith("gl_"),
      ),
      builtinVars: Object.keys(parsedOpenGlDocs).filter((k) =>
        k.startsWith("gl_"),
      ),
