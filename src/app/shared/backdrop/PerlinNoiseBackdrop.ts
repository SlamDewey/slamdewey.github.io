...
float fractal_perlin(vec4 x) {
  float val = 0.;
  float scale = 0.5;
  float magnitude = 0.;
  float f = FREQUENCY;
  for( int i = 0; i < OCTAVES; i++ ) {
    val += scale * cnoise(x * f);
    magnitude += scale;
    scale *= PERSISTANCE;
    f *= LACUNARITY;
  }
...
void main() {
  ...
  // get coords for noise input
  vec4 q = vec4(
    uv.xy * ZOOM,
    RADIUS * cos(totalTime * TIME_SCALAR / RADIUS),
    RADIUS * sin(totalTime * TIME_SCALAR / RADIUS)
  );
  // get noise at location
  float f = fractal_perlin(q);
